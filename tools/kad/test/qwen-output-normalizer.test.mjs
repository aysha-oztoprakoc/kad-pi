...
// T19 frozen upstream resume avoids another controller call
 test('T19 frozen upstream resume avoids another controller call', async () => {
  ...
  assert.equal(result.status, 'ACCEPTED'); assert.equal(executeCalls, 0); assert.equal(consumeCalls, 0); assert.equal(result.telemetry.new_remote_controller_calls, 0); assert.equal(result.telemetry.incremental_remote_tokens, 0); assert.equal(result.telemetry.inherited_parent.remote_tokens, 18); assert.equal(result.telemetry.controller_invocations, 1);
 });
