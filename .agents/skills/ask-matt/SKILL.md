...
- **KAD high-impact decision** → **`/kad-advisory-board`** for the five-lens advisory review, then **`/wayfinder`** for the human decision ticket.
- **KAD large/foggy effort** → **`/wayfinder`** with `.agents/skill-overlays/wayfinder-kad.md` loaded.
- **KAD evidence-dependent decision** → **`/research`** for facts, then return the value decision to **`/wayfinder`**; research never decides for the human.
- **KAD implementation** → preserve **`/to-spec` → `/to-tickets` → `/implement` → `/tdd` → `/code-review`**; `workctl` remains the execution owner.
